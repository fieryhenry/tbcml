...
class FridaScript:
    ...
    @staticmethod
    def from_zip(zip: "io.zip.Zip", arc: str, cc: "country_code.CountryCode", gv: "game_version.GameVersion") -> "FridaScript":
...
class Scripts:
    def __init__(self, scripts: list["FridaScript"], cc: Optional["country_code.CountryCode"] = None, gv: Optional["game_version.GameVersion"] = None):
        ...
        if len(scripts) > 0:
            ...
            if cc is None or gv is None:
                raise ValueError("Country code and game version must be specified if no scripts are provided.")
            self.cc = cc
            self.gv = gv
    ...
    def validate_scripts(self):
        for script in self.scripts:
            if not self.is_valid_script(script):
                raise ValueError("Script is not valid for this game version and country code.")
    ...
    def add_to_zip(self, zip: "io.zip.Zip"):
        ...
        json = io.json_file.JsonFile.from_json(json_data)
        zip.add_file(io.path.Path("scripts/scripts.json"), json.to_data())
        
    @staticmethod
    def from_zip(zip: "io.zip.Zip", cc: "country_code.CountryCode", gv: "game_version.GameVersion") -> "Scripts":
