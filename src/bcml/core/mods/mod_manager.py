...
class ModManager:
    ...
    def load_mods(self):
        for path in self.mod_folder.get_files():
            ...
            if mod is not None:
                self.mods[mod.get_full_mod_name()] = mod
    ...
    def init_new_json(self):
        self.json_file = io.json_file.JsonFile.from_json({
            "mods": {}
        })
    ...
    def enable_mod(self, mod: bc_mod.Mod):
        self.save_mod(mod)
        self.json_file.get_json()["mods"][mod.get_full_mod_name()] = True
        self.save_mod_json()
    
    def disable_mod(self, mod: bc_mod.Mod):
        self.save_mod(mod)
        self.json_file.get_json()["mods"][mod.get_full_mod_name()] = False
        self.save_mod_json()
    
    def is_mod_enabled(self, mod: bc_mod.Mod) -> bool:
        if mod.get_full_mod_name() not in self.json_file.get_json()["mods"]:
            return True
        return self.json_file.get_json()["mods"][mod.get_full_mod_name()]
    ...
    def increase_priority(self, mod: bc_mod.Mod):
        if mod.get_full_mod_name() not in self.json_file.get_json()["mods"]:
            return
        self.json_file.get_json()["mods"].insert(0, mod.get_full_mod_name(), self.json_file.get_json()["mods"].pop(mod.get_full_mod_name()))
        self.save_mod_json()
    
    def decrease_priority(self, mod: bc_mod.Mod):
        if mod.get_full_mod_name() not in self.json_file.get_json()["mods"]:
            return
        self.json_file.get_json()["mods"].append(mod.get_full_mod_name(), self.json_file.get_json()["mods"].pop(mod.get_full_mod_name()))
        self.save_mod_json()

    def get_mods(self) -> list[bc_mod.Mod]:
        return list(self.mods.values())
    
    def get_mod(self, name: str) -> Optional[bc_mod.Mod]:
    ...
    def save_mod(self, mod: bc_mod.Mod):
        self.mods[mod.get_full_mod_name()] = mod
        mod.save(self.mod_folder.add(mod.get_full_mod_name()))
    
    def remove_mod(self, mod: bc_mod.Mod):
        self.mods.pop(mod.get_full_mod_name())
        self.mod_folder.add(mod.get_full_mod_name()).remove()
