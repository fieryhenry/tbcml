...
class BCImage:
    ...
    def scale(self, scale: float):
        self.__image = self.image.resize((int(self.width * scale), int(self.height * scale)), resample=Image.BICUBIC)
    ...
    def draw_circle(self, x: int, y: int, radius: int, color: tuple[int, int, int, int]):
