...
class Stats:
    ...
    def assign(self, raw_data: list[int]):
        ...
        self.wave = unit.Wave.from_values(raw_data[27], raw_data[28], bool(raw_data[86]))
        ...
        self.warp = unit.Warp.from_values(raw_data[65], raw_data[66], raw_data[67], raw_data[68])
        ...
        self.surge = unit.Surge.from_values(raw_data[81], raw_data[82], raw_data[83], raw_data[84])
        self.surge_immunity = bool(raw_data[85])
        self.shield = unit.Shield.from_values(raw_data[87], raw_data[88])
        self.death_surge = unit.Surge.from_values(raw_data[89], raw_data[90], raw_data[91], raw_data[92])
    ...
    def to_raw_data(self) -> list[int]:
        return [
            ...
            self.strengthen.hp,                         # 32
            self.strengthen.multiplier,                 # 33
...
class StatsData:
    ...
    def serialize(self) -> dict[str, Any]:
        return {
            "stats": {str(enemy_id): stats.serialize() for enemy_id, stats in self.stats.items()},
        }
    
    @staticmethod
    def deserialize(data: dict[str, Any]) -> "StatsData":
        return StatsData(
            {int(enemy_id): Stats.deserialize(stats_data, int(enemy_id)) for enemy_id, stats_data in data["stats"].items()},
        )
...
class Anim:
    ...
    def get_maanim_path(enemy_id: int, anim_type: "bc_anim.AnimType") -> str:
        ...
        return Anim.get_img_path(enemy_id).replace(
    ...
    @staticmethod
    def from_game_data(
        game_data: "pack.GamePacks", enemy_id: int
    ) -> Optional["Anim"]:
...
class Descriptions:
    ...
    def from_game_data(
        ...
        if file is None:
            return None
        csv = io.bc_csv.CSV(file.dec_data, delimeter=io.bc_csv.Delimeter.from_country_code_res(cc), remove_empty=False)
        for enemy_id, line in enumerate(csv.lines):
...
class Enemy:
    ...
    @staticmethod
    def from_game_data(
        game_data: "pack.GamePacks", enemy_id: Optional[int], stat_data: StatsData, names: Names, descriptions: Descriptions, release_id: Optional[int] = None
    ) -> Optional["Enemy"]:
        if release_id is not None:
            enemy_id = release_id - 2
        if enemy_id is None:
    ...
    def to_game_data(self, game_data: "pack.GamePacks"):
        ...
        game_data.set_file(Enemy.get_enemy_icon_name(self.enemy_id), self.enemy_icon.to_data())
...
class Enemies:
    ...
    def serialize(self) -> dict[str, Any]:
        return {
            "enemies": {str(enemy_id): enemy.serialize() for enemy_id, enemy in self.enemies.items()}
        }
    ...
    def from_game_data(game_data: "pack.GamePacks") -> Optional["Enemies"]:
        ...
        for enemy_id in names.names.keys():
            enemy = Enemy.from_game_data(game_data, enemy_id, stats, names, descriptions)
            if enemy is not None:
                enemies[enemy_id] = enemy
        return Enemies(enemies)
    
    def to_game_data(self, game_data: "pack.GamePacks"):
        stats = StatsData({enemy.enemy_id: enemy.stats for enemy in self.enemies.values()})
        names = Names({enemy.enemy_id: enemy.name for enemy in self.enemies.values()})
        descriptions = Descriptions({enemy.enemy_id: enemy.description for enemy in self.enemies.values()})
    ...
    @staticmethod
    def from_zip(zip: "io.zip.Zip") -> Optional["Enemies"]:
