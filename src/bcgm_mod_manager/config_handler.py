...
def get_config_path() -> str:
    ...
    """
    return os.path.join(get_app_data_folder(), "config.yaml")
...
update_to_betas: false # Whether to update to betas
"""
    if not os.path.exists(config_file):
...
def set_update_to_betas() -> None:
    """
    Sets whether to update to betas.
    """
    update_to_betas = helper.colored_input(
        "Enter whether to update to betas (true/false):"
    )
    set_config_setting("update_to_betas", update_to_betas)
    helper.colored_text("Update to betas setting set successfully.", helper.Color.GREEN)    