...
def make_image(size: tuple[int, int]) -> bytes:
    """
    Make a 128x128 transparent png
    """
    img = Image.new("RGBA", size, (0, 0, 0, 0)).convert("RGBA", colors=2**32)
    return add_enemy_as_cat.save_image(img)
...
def insert_deploy_image(deploy_path: str) -> bytes:
    ...
    data = add_enemy_as_cat.save_image(transparent_img)
    return data


def insert_display_image(display_path: str) -> bytes:
    transparent_data = make_image((512, 128))
    ...
    data = add_enemy_as_cat.save_image(transparent_img)
    data = make_display_bottom_right_transparent(data)
    if data is None:
        return transparent_data
    return data


def scale_height_display_file(display_path: str, sf: float = 3.5) -> Optional[bytes]:
    """
    Scale the display file by sf * the height
    """
    ...
    try:
        ...
        return None
    new_height = int(display_img.height * sf)
    new_width = int(display_img.width * sf)
    display_img = display_img.resize((new_width, new_height), Image.ANTIALIAS)
    data = add_enemy_as_cat.save_image(display_img)
    return data


def make_display_bottom_right_transparent(image_data: bytes):
    """
    Make the bottom right corner of the display transparent
    """
    display_img = Image.open(io.BytesIO(image_data))
    start_pos = (146, 112)
    end_pos = (188, 70)
    start_offset = 0
...
def make_deploy_file(
    editor: game_file_editor.GameFileEditor, dir_name: str, cat_id: int, form_id: int
):
    """
...
def get_cat_id(
    editor: game_file_editor.GameFileEditor, dir_name: str
) -> tuple[int, int]:
    """
...
def import_units(pack_data: Any, animations_dir: str) -> None:
    ...
    for unit_data in units:
        ...
        import_sprite(
            editor, os.path.join(animations_dir, unit_anim_name), cat_id, form
        )
...
def get_unit_csv_data(unit_data: dict[str, Any]) -> list[int]:
    ...
    csv_data: list[int] = [
        ...
        max(
            get_proc_prob(procs, "WAVE"), get_proc_prob(procs, "MINIWAVE")
        ),  # wave probability
        max(
            get_proc_level(procs, "WAVE"), get_proc_level(procs, "MINIWAVE")
        ),  # wave level
        ...
        1
        if get_attack(base_stats["atks"]["pool"], 1, "ld1") != 0
        else 0,  # second attack long distance flag
        get_attack(base_stats["atks"]["pool"], 1, "ld0"),  # long distance start 2
        get_attack(base_stats["atks"]["pool"], 1, "ld1"),  # long distance range 2
        1
        if get_attack(base_stats["atks"]["pool"], 2, "ld1") != 0
        else 0,  # third attack long distance flag
