...
class AudioFile:
    ...
    def __init__(self, data: "tbcml.Data", file_name: str):
        """Initializes an AudioFile.

        Args:
            data (tbcml.Data): The data of the audio file.
    ...
    @staticmethod
    def from_zip(zip_file: "tbcml.Zip", file_name: str) -> "AudioFile":
        """Creates an AudioFile from a mod zip file.

        Args:
            zip_file (tbcml.Zip): Mod zip file to read from.
        ...
        """
        path = tbcml.Path("audio").add(file_name)
        data = zip_file.get_file(path)
        if data is None:
            raise ValueError(f"File {path} not found in zip file")
        return AudioFile(data, file_name)

    def to_zip(self, zip_file: "tbcml.Zip"):
        """Adds the audio file to a mod zip file.

        Args:
            zip_file (tbcml.Zip): Mod zip file to add to.
        """
        path = tbcml.Path("audio").add(self.file_name)
    ...
    @staticmethod
    def from_file(path: "tbcml.Path") -> "AudioFile":
        """Creates an AudioFile from a file.

        Args:
            path (tbcml.Path): Path to the audio file.
    ...
    def play(self):
        ...
        with tbcml.TempFile(extension=self.get_extension()) as temp_file:
...
class Audio:
    ...
    @staticmethod
    def from_zip(zip_file: "tbcml.Zip") -> "Audio":
